...
# ╔═╡ a14aae80-f294-47cf-b9cd-a604de7f6d95
using OrdinaryDiffEq, Plots, PlutoUI, FFTW, DSP

# ╔═╡ 3131b870-ffbc-11ed-388a-b55bbe719b44
md"""# Single mode clarinet

Using complex model for resonator (from F. Silva)"""

# ╔═╡ f66c8f23-4716-4e1e-a534-78613ec13f55
...
# ╔═╡ d63d4af1-993d-47b1-98b8-aac17f513112
...
# ╔═╡ eee7ebdb-b7d2-42f8-9062-a65b061bf684
plot(nlstiff)

# ╔═╡ f69c6b27-b5ec-40aa-b482-b4505929d096
md"## Constants"

# ╔═╡ 4d835a2e-23af-41ad-b346-25394c658db6
md"""
## Test the resonator with a click
"""

# ╔═╡ c9e3f2de-0622-42a4-b8e3-44751a538e6c
md"## Define the problem"

# ╔═╡ 63596ff2-e2b0-4781-8063-023805a7f1b6
plotly()

# ╔═╡ fb7cd9d0-4445-4d87-bf41-824976887e5b
@bind γ confirm(Slider(0:.01:2))

# ╔═╡ e1441518-06bf-4c1e-8796-dec47cf76974
@bind ζ confirm(Slider(0:.01:2))

# ╔═╡ 0be8e53c-8345-4823-9aca-845711758bc0
@bind ω₁ Slider(0:10:6000)

# ╔═╡ 46fc61f5-e47b-4c33-8b4c-83a68e009038
begin
	α = 0.001
	# resonator
...
# ╔═╡ d59f75f2-7bd8-4056-bba2-20060b27001b
s, c = modalPoleRes(ω₁, q₁, a=q₁)

# ╔═╡ 152245f1-1f42-4762-80f7-fb53029a5927
function resonator(du,u,par,t)
	du[1] = c * par[1](t) + s * u[1]
end

# ╔═╡ ad52bde2-9ba1-447d-919c-bb04bedfc9fb
begin
...
# ╔═╡ 2193ef1a-e571-40b5-adc2-91cc6f6a8bcb
...
# ╔═╡ a4704d2f-6ae8-4a35-aeba-7b1e79024e35
...
# ╔═╡ 3f2fc222-8ad5-4669-92ea-fab3b0bad5b0
begin
...
# ╔═╡ ca1c38d1-ada2-440b-852e-ddd9b4a0b57a
begin
	dtf = 0.0001
	nf = 1024
	tf = 0:dtf:dtf*(nf-1)
...
# ╔═╡ ff842f74-0fff-4865-8d3e-6700f21d15f9
ω₁/2/pi

# ╔═╡ 2f572269-59b1-416e-bf85-5425fe54b50e
γ
...
# ╔═╡ e7ef59f7-52dd-4b59-a6a6-3b16453c21d1
begin
	N=length(sol.u)
	J = length(sol.u[1])
	u = hcat(sol.u...)
...
# ╔═╡ 3fe4a378-cacb-4518-b279-5bb5bad44f63
per=DSP.welch_pgram(psol, fs=1/dt, window=hanning)

# ╔═╡ 318cd8cd-fe1f-4b9a-b8c6-92c0299eb2e1
begin
	plot(per.freq,10 .* log10.(per.power))
	vline!([ω₁/2/pi],lw=2)
	#xlims!(0,ω₁/1)
end

# ╔═╡ 8d2e54df-9898-433a-a204-5ee410deb0b0
ω₁/2/pi

# ╔═╡ 248ebbf2-3a56-4701-b5f0-c24b223d5893
length(psol)

# ╔═╡ f074d297-9cd9-45f9-931a-e957e47a545b
function findPeaks(x)
	pkmask = (x[1:end-2] .< x[2:end-1]) .& (x[3:end] .< x[2:end-1])
	pkmask = [false; pkmask; false]
end

# ╔═╡ d83164bd-1dfb-4e95-a573-1260d762a57a
function highestNPeaks(x; n=5)
	pmask = findPeaks(x)
	pkIdx = findall(pmask)
	sortIdx = sortperm(x[pmask])
	pkIdx[sortIdx[end:-1:end-n]]
end

# ╔═╡ afa72999-e5d7-4609-bf25-ee100fb05e88
function pvocFrame(x; nfft=1024, lag=256, window=nothing, npk=10)
...
# ╔═╡ 24cef554-47a5-459c-95f0-1b8f8a04ef2b
nuttall

# ╔═╡ 6da6099c-0d1a-4419-9a5a-862dc044b817
begin
	lag=256
	nfft=1024
end

# ╔═╡ 30d9f902-652a-42e2-84cd-1acaa7ddbe2f
frames = pvocFrame(psol;nfft=nfft,lag=lag,window=hanning)

# ╔═╡ f2e8bcbd-554f-4748-8cf8-e27fb447e851
frav = sum((abs.(frames)).^2, dims=2)

# ╔═╡ 27cf93fd-1c2b-460f-91ac-aca802a16121


# ╔═╡ 3b2d0541-53ed-4a3b-9dd1-ca7ccd761822
pki = highestNPeaks(frav) 

# ╔═╡ 5dbdbea5-6e10-44b7-9ba6-bd8956a25588
begin
	plot(log.(frav))
	vline!(pki)
end

# ╔═╡ 4c7ecbc9-dcda-4f7c-9004-4aa131a94fe6
bno=pki[3]

# ╔═╡ 2d35014a-dcf0-47fe-b6e5-2a016084e83a


# ╔═╡ 7ad6ee0e-1f3d-43d3-9cd6-269e7fdb7d01
dphi = diff(angle.(frames[pki,:]),dims=2)

# ╔═╡ 2448a012-0acb-4d64-818d-784cc79fdeb0
angle.(frames[bno,:])

# ╔═╡ 10ec02b2-c2a6-4165-87f8-65b173b81dc8
dphi/lag/2pi

# ╔═╡ 51a89fc3-96c4-40fc-aae5-ba6bfa7e8910
fbin = (pki.-1)./nfft

# ╔═╡ d1cd606b-fc94-4a00-95d8-0502267b1da8
nwrap = round.(fbin.*lag)

# ╔═╡ bd5c5fef-f6a7-47f5-9544-ec31d7f6950d
fopt = (dphi/2pi .+ nwrap .+ (-1:1)' )./lag

# ╔═╡ 8e05587d-e0e5-4c42-b4f7-72369d058b56
dphi/2pi .+ nwrap .+ (-1:1)'

# ╔═╡ 4cf25e0f-db94-472e-9da4-fc7c110df9c2
fbin*lag

# ╔═╡ 6db63344-9ed4-4a05-8feb-0a4e0e05c0b6
fbin/dt

# ╔═╡ 8460c969-05bc-4147-8754-14c5fd33c35f
fopt[argmin(abs.((fbin.-fopt)))]/dt

# ╔═╡ b2fa1ca1-fd12-4a8b-9a88-ee1623a3c2ca
fopt[argmin(abs.(fbin.-fopt),dims=2)]./dt
...
manifest_format = "2.0"
project_hash = "b3edcd89a81bc751557f3961bd170e9c0b62e25e"
